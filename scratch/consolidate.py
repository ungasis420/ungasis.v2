...
# Separate footer
if "\n---\n" in claude_content:
    parts = claude_content.split("\n---\n")
    main_content = "\n---\n".join(parts[:-1])
    footer = "\n---\n" + parts[-1]
else:
    main_content = claude_content
    footer = ""
...
# Append and save CLAUDE.md
final_claude = main_content.rstrip() + "\n\n" + new_content.strip() + footer
with open(claude_path, 'w', encoding='utf-8') as f:
    f.write(final_claude)

print("CLAUDE.md updated successfully.")

# Prepend header to .agents/rules/ files
header = """# ⚠️ SOURCE OF TRUTH: CLAUDE.md
# This file is a reference copy. All rules are maintained in CLAUDE.md.
# Last synced: 2026-06-10

"""

rules_dir = os.path.join(base, '.agents', 'rules')
count = 0
for fp in glob.glob(os.path.join(rules_dir, '*.md')):
    with open(fp, 'r', encoding='utf-8') as f:
        content = f.read()
    if not content.startswith("# ⚠️ SOURCE OF TRUTH"):
        with open(fp, 'w', encoding='utf-8') as f:
            f.write(header + content)
        count += 1

print(f"Header prepended to {count} files in .agents/rules/.")

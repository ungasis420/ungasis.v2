...
<!-- ADDED: mattpocock/skills absorption — Caveman Mode -->

## 18B. Caveman Mode — Emergency Token Saver 🦴

### What Is Caveman Mode?

Caveman Mode is a communication mode where AI drops ALL filler words, pleasantries, unnecessary explanation, and verbose padding. Output becomes ultra-compressed while keeping 100% technical accuracy.

**🍳 Analogy:** Normal AI is like a waiter who says: "Good evening! Welcome to our wonderful restaurant. I'd love to help you today. Based on my careful analysis of the menu, I believe the chicken adobo would be an excellent choice for you because of its rich flavor profile and..." Caveman Mode is like a waiter who says: "Adobo. Best dish. Table 3." Same information. 75% fewer words.

### Before/After Example

| Mode | Output | ~Tokens |
|---|---|---|
| **Normal** | "I've analyzed your codebase and I believe the issue is located in the authentication middleware. The problem appears to be that the token validation function is not properly handling expired tokens. Here's what I'd recommend as a solution..." | ~45 tokens |
| **🦴 Caveman** | "Bug: auth middleware. Token validation skips expired check. Fix: add expiry check in validateToken(). Line 47." | ~20 tokens |

**Result:** Same fix. ~75% fewer tokens. No information lost.

### More Before/After Examples for Your Work

| Situation | Normal AI | 🦴 Caveman AI |
|---|---|---|
| **File structure advice** | "Based on my analysis of your project structure, I would recommend organizing your files in the following manner to improve maintainability and developer experience..." | "Structure: `/css` tokens+base+layout+components. `/js` one file per feature. `/data` one file per dataset. Done." |
| **Bug diagnosis** | "After carefully reviewing the error message you provided, it appears that the issue stems from a missing import statement in your main component file..." | "Missing import. `App.jsx` line 12. Add `import Chart from './Chart'`. Rebuild." |
| **Decision question** | "That's a great question! There are several factors to consider when choosing between Option A and Option B. Let me walk you through the pros and cons of each approach..." | "Option A. Reason: free, simpler, fits your stack. B needs paid API. Go A." |

### When to Use Caveman Mode ✅

| Situation | Why Caveman Helps |
|---|---|
| Context window is running low (long session) | Saves remaining tokens for actual work |
| Doing repetitive tasks (edit file 1, edit file 2, edit file 3...) | You already understand the pattern — skip the explanation |
| AI is being too verbose and you need just the answer | Cuts the fluff instantly |
| Multi-agent handoff (passing output to another AI) | AI does not need pleasantries — just data |
| Token budget is tight (API calls, free-tier limits) | Every saved token = more useful output |
| Cline/coding tasks where you just need the fix | Code does not need a motivational speech |

### When NOT to Use Caveman Mode ❌

| Situation | Why Normal Mode Is Better |
|---|---|
| Learning a new concept for the first time | You NEED the explanation, analogies, and examples |
| Writing documentation for humans | Humans need readable, complete sentences |
| Explaining something to someone else | Compressed output confuses beginners |
| Making important decisions | You need full reasoning to evaluate properly |
| Writing emails, reports, or presentations | Professional communication needs proper tone |
| When you are confused and need clarity | Caveman output can be TOO compressed when you are lost |
...
Rules:
- Drop ALL filler words, pleasantries, and unnecessary explanation.
- No "Great question!", no "I'd be happy to help!", no "Here's what I think..."
- Ultra-compressed output. Maximum information, minimum tokens.
- Keep technical accuracy at 100%.
- Use short sentences. sentence fragments OK. tables OK. bullet OK.
- Skip transitions and summaries unless I ask.
- Code: just the fix. No explanation unless I ask "why".
- If I ask a yes/no question, answer yes or no first, then reason in <10 words.

Stay in Caveman Mode until I say "EXIT CAVEMAN MODE" or "NORMAL MODE".
```

### 🗣️ Copy-Paste Deactivation Prompt

```text
EXIT CAVEMAN MODE. Return to normal detailed responses with full explanations, 
analogies, tables, and beginner-friendly language.
```

### Caveman Mode Quick Reference Card

```
ACTIVATE:  "ACTIVATE CAVEMAN MODE" or "🦴 caveman on"
USE WHEN:  low tokens, repetitive tasks, verbose AI, coding fixes, agent handoffs
AVOID WHEN: learning, documenting, explaining, deciding, confused
DEACTIVATE: "EXIT CAVEMAN MODE" or "🦴 caveman off" or "NORMAL MODE"
SAVINGS:   ~60-75% token reduction on typical outputs
```

### Combining Caveman Mode with Other Token Savers

Caveman Mode works even better when stacked with other techniques from Section 18:

| Technique | Alone | + Caveman Mode |
|---|---|---|
| Ask for tables instead of paragraphs | ~30% savings | ~60% savings |
| One task per prompt (decomposition) | ~20% savings | ~50% savings |
| Skip repeating context AI already has | ~25% savings | ~65% savings |
| Use output format constraints | ~20% savings | ~55% savings |
| **All combined** | ~40-50% savings | **~70-80% savings** |

### ⚠️ Safety Note

Caveman Mode reduces OUTPUT tokens, not your instruction quality. Your prompts should still be clear and complete — Caveman Mode only changes how AI responds, not how you ask.

**Do NOT use Caveman Mode to skip safety checks.** Even in Caveman Mode:
- AI must still flag security risks
- AI must still warn about secrets
- AI must still ask before destructive actions
- AI must still mark unverified claims

### 🍳 Analogy: When to Talk Normal vs Caveman

Think of it like texting:
- **Normal Mode** = writing a proper email to your manager. Full sentences, context, politeness. ✉️
- **Caveman Mode** = texting your best friend who already knows the situation. "line 47. expired token. fixed." 📱

Both communicate the same thing. You pick the mode based on WHO is reading and HOW MUCH context they already have.

<!-- END ADDED: mattpocock/skills absorption — Caveman Mode -->

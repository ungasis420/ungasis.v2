// src/components/build/CoreItemsTable.tsx
"use client";
...
// Component
// ──────────────────────────────────
export function CoreItemsTable({ items, reasoningLoading }: CoreItemsTableProps) {
  if (!items || items.length === 0) return null;

  return (
    <div>
      <h5 className="text-base font-semibold text-white/90 mb-3">
        🗡️ Core Build ({items.length} items)
      </h5>

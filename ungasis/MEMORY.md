...
| June 2026 | Archive old ungasis-os/ folder | Clean repo, prevent Cline scanning waste | ✅ Done |
| June 2026 | Absorb drona23 output token rules | 63% output reduction benchmarked | ✅ Added as Layer 13 |
...
| Opus hits context limits at 12+ tasks | Split into max 7 per session | ✅ High |
| QA audit fails if not all files are attached | Always verify file count before audit | ✅ High |
| UNGASIS optimized INPUT tokens but not OUTPUT | Added Layer 13 for output optimization | ✅ High |
| "Thank you" messages cost full context resend | Never send social messages to AI agents | ✅ High |
| 3 failed attempts = poisoned context | Reset session after 3 failures | ✅ High |
| Edit and resend is cheaper than correction messages | Prevents context noise accumulation | ✅ High |
...
| June 2026 | 5 bonus files from 1st Opus lost | Context window hit limit, files not saved | ✅ Max 7 tasks per session rule |
| June 2026 | QA bot said 82% coverage, actual was 93% | Bot didn't have the module files | ✅ Always verify what files the bot received |
...
| Max 7 tasks per Opus session | All AI delegation | Prevents context overflow |
| Always say "next" between tasks | All bot sessions | Pacing control |
| Save outputs immediately after generation | All generation tasks | Don't rely on session persistence |
| Attach ALL relevant files to QA bots | All QA/audit tasks | Prevents false negatives |
| Use IDE agent for file-heavy audits | QA tasks with 10+ files | Cline reads from disk, no attachment limits |
...
| M365 Copilot ChatGPT | Structured frameworks | ✅ Strong | ✅ Fast | Good for matrices and specs |
...
| QA in separate session | Keeps coordinator context clean | ✅ Yes |
| Pre-written kickoff prompts | Saves delegation time, ensures consistency | ✅ Yes |
| .clinerules/ folder structure | Cline auto-reads, cached across turns | ✅ Yes |
| Numbered task files (00, 01, 02...) | Clear priority order for agent loading | ✅ Yes |
...
| Sending "thank you" to AI agents | Costs full context resend (~50K tokens) | Just start next task |
| Editing .clinerules/ mid-session | Breaks cache, wastes tokens | Edit between sessions only |
| Retrying same approach 4+ times | Poisoned context, diminishing returns | 3-attempt rule: reset session |
| Sending correction messages | Appends noise to context | Edit original and resend |

---

<!-- ABSORBED: Tool bookmarks and new table rows added from drona23/claude-token-efficient (5.5K stars) + Habib Mohammed "10 Tips to Stop Burning Tokens in Claude Code" (Medium) + community research. Absorbed June 2026 by Mel John Dimat. -->

### Tools to Revisit (Bookmarked for Future)

| Tool | Stars | What It Does | When to Revisit | Why Not Now |
|------|-------|-------------|----------------|-------------|
| HydraMCP (Pickle-Pixel) | 15 | MCP server querying multiple LLMs from one terminal - compare, consensus, synthesize modes | When adopting Claude Code CLI | Requires Node.js + Claude Code CLI |
| HydraTeams (Pickle-Pixel) | 64 | Translation proxy - Claude Code Agent Teams works with any model (GPT, Gemini, Ollama) | When adopting Claude Code CLI | Requires Claude Code + Node.js + TypeScript |
| Graphify | 57.7K | Turns code/docs into queryable knowledge graph via /graphify command | When codebase > 20 files needing deep search | Requires pip install + Python 3.10+ |
| AgentMemory | 20.4K | Knowledge graph + hybrid search memory server via MCP | When building production AI app | Requires Node.js server + database |
| Claude-Mem | 78.6K | Persistent memory across sessions for any agent via MCP | When building production AI app | Requires MCP server setup |
| drona23/claude-token-efficient | 5.5K | CLAUDE.md rules for 63% output token reduction | ✅ ABSORBED into Layer 13 | Already integrated |
| Google Cloud Workstations noVNC | 9 | Docker cloud desktop with GNOME + browser GUI | Never | GitHub Codespaces already solves this for free |

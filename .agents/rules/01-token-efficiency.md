...
### L13 — Graphify-First Querying
- For ANY codebase question → query graphify-out/graph.json FIRST
- Only read raw files if graph doesn't answer
- Saves 70x+ tokens per query (41K nodes indexed)
- Never full rebuild — incremental update only
